...
export class OriginValidator {
  ...
  constructor(allowedOrigins = null, logger) {
    ...
    if (allowedOrigins === null) {
      ...
      this.allowedOrigins = [window.location.origin]
      this.logger.warn(
        'Invalid allowedOrigins provided, defaulting to same-origin only'
      )
    }
  }
  ...
  validate(origin) {
    ...
    if (this.allowedOrigins.includes('*')) {
      this.logger.warn(
        'Wildcard origin validation - accepting all origins (security risk)'
      )
      return true
    }
    ...
    if (!isAllowed) {
      this.logger.error(ERRORS.ORIGIN_MISMATCH, {
        origin,
